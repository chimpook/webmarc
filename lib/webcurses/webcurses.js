...
*/
        ...
        wfill: function (win, ch, bg, fg, attr) {
            var self = this;
            var row, col;

            attr = attr === undefined ? 0 : attr;
            for (row = 0; row < self.Windows[win].nlines; row++) {
                for (col = 0; col < self.Windows[win].ncols; col++) {
                    self.Windows[win].ram[row][col] = {
                        ch: ch,
                        bg: bg,
                        fg: fg,
                        attr: attr
        ...
        initscr: function (container) {
            ...
            // Создаем умолчательное окно
            self.newwin('stdscr', self.SCREEN_H, self.SCREEN_W, 0, 0);

            self.fill('@', self.COLOR_DARKRED, self.COLOR_LIGHTYELLOW);
            self.printw('stdscr');

            // Тестовое окно
            self.newwin('test', 12, 12, 5, 4);

            self.wfill(1, '@', self.COLOR_DARKBLUE, self.COLOR_CYAN);
            self.wprintw(1, 'test');

            //console.log(self.Windows);

            // Компонуем "видеопамять"
            self.compose();
            ...
            container.append('<div id="' + self.wss + '">' + self.buffer.html + '</div>');
