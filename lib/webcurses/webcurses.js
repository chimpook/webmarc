...
(function () {
    ...
    Webcurses.prototype = {
        ...
        // Габариты контейнера и рабочего пространства
        container_height: 0,
        container_width: 0,
        ws_height: 0,
        ws_width: 0,

        // "Системная видеопамять"
        ram: [],
        ...
        newwin: function(title, nlines, ncols, begin_y, begin_x, layer) {
            ...
            nlines  = Math.round(nlines);
            ncols   = Math.round(ncols);
            begin_y = Math.round(begin_y);
            begin_x = Math.round(begin_x);
...
*/
            ...
            if (error) {
                console.log("Error creating new window");
                return;
            }

            self.Windows[layer] = new WebcursesWindow(title, nlines, ncols, begin_y, begin_x, layer);
            self.Windows[layer].fill(' ', self.COLOR_BLACK, self.COLOR_WHITE);
        ...
        initscr: function (container) {
            ...
            self.container_width = container_w;
            self.container_height = container_h;
            ...
            /*
             // Подгонка по высоте
             var font_h = container_h / this.LINES ^ 0; // -LINES - пока включена сетка
             var font_w = font_h * this.FONT_AJUST;
             if ((font_w + this.GRID) * this.COLS > container_w) {
             // Подгонка по ширине
             //console.log("font too big");
             font_w = (container_w - this.GRID) / this.COLS ^ 0;
            ...
            self.Windows[0] = new WebcursesWindow('stdscr', self.LINES, self.COLS, 0, 0, 0);
            self.Windows[0].fill(' ', self.COLOR_BLACK, self.COLOR_WHITE);
            ...
            self.ws_height = ws.height();
            self.ws_width = ws.width();
        },
        ...
        makebacklink: function (link) {
            var self = this;

            $("body").append('<div class="back" style="position: absolute; top: ' + self.ws_height + 'px;">' +
                '<a href="' + link + '" style="color: royalblue;">Back to Index</a>' +
                '</div>');
        }
    };
